...
class SearchNotesIndexItem extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
  ...
  render() {
    ...
    return (
      <Link to={`/note/search/${this.props.match.params.search}/${this.state.note.id}`}
        className={cn}>
...
export default withRouter(SearchNotesIndexItem);
